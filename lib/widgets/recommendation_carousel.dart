...
class RecommendationCarousel extends StatefulWidget {
  ...
  const RecommendationCarousel({super.key, this.data});
...
class _RecommendationCarouselState extends State<RecommendationCarousel> {
  ...
  final Set<int> _dismissedMovieIds = {};
  ...
  void _showQuickActions(BuildContext context, MovieModel movie) {
    showDialog(
      context: context,
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ClipRRect(
                    borderRadius: BorderRadius.circular(2),
                    child: Image.network(movie.posterPath, height: 160, fit: BoxFit.cover),
                  ),
                  ...
                  const Divider(color: Color(0xFF111111), height: 1),
                  _buildActionButton(context, icon: Icons.not_interested_rounded,
  ...
  Widget _buildActionButton(BuildContext context, {required IconData icon, required String label, required VoidCallback onTap, Color? color}) {
    return InkWell(
      ...
      child: Padding(
          ...
          child: Row(children: [
            Icon(icon, color: color ?? const Color(0xFFC62828), size: 20),
            const SizedBox(width: 15),
            Text(label, style: TextStyle(color: color ?? const Color(0xFF111111), fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 2))
          ])),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final List<MovieModel> movies = (widget.data!['movies'] as List)
        .map((m) => m is MovieModel ? m : MovieModel.fromJson(m))
        .where((m) => !_dismissedMovieIds.contains(m.id))
        .toList();
        ...
        child: Center(
          child: Column(
            ...
            children: const [
              ...
              SizedBox(height: 6),
              Text("Rate more movies to improve your Archival DNA",
        ...
        children: [
          SizedBox(
            ...
            child: PageView.builder(
              ...
              itemBuilder: (context, index) => _buildEyeCatchyCard(movies[index], index),
            ),
          ),
          const SizedBox(height: 12),
          Center(child: _FilmStripIndicator(offset: _currentPage, itemCount: movies.length)),
        ],
      ),
    );
  }
  ...
  Widget _buildEyeCatchyCard(MovieModel movie, int index) {
    ...
    if (movie.director != null && currentTitle.contains(movie.director!.split(' ').last.toUpperCase())) {
    ...
    for (var dial in MovieDialogue.dialogues) {
      if (dial.movieTitle.toUpperCase() == movie.title.toUpperCase() || dial.tmdbId == movie.id) {
    ...
    if (displayQuote.isEmpty) {
      displayQuote = movie.overview.isNotEmpty ? movie.overview : "Experience this pure form of cinema.";
      displayChar = movie.director?.toUpperCase() ?? "ARCHIVAL CLASSIC";
    }
...
class _FilmStripIndicator extends StatelessWidget {
  final double offset; final int itemCount;
  const _FilmStripIndicator({required this.offset, required this.itemCount});
  @override
  Widget build(BuildContext context) {
    return SizedBox(height: 30, width: 200, child: CustomPaint(painter: _FilmStripPainter(offset: offset, itemCount: itemCount)));
  }
}

class _FilmStripPainter extends CustomPainter {
  final double offset; final int itemCount;
  _FilmStripPainter({required this.offset, required this.itemCount});
  @override
  void paint(Canvas canvas, Size size) {
    ...
    final sprocketPaint = Paint()..color = Colors.black.withOpacity(0.15)..shader = fadeShader;
    ...
    for (int i = 0; i < 20; i++) {
      ...
      canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(x, 4, 6, 4), const Radius.circular(1)), sprocketPaint);
      canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(x, size.height - 8, 6, 4), const Radius.circular(1)), sprocketPaint);
    }
  ...
  @override bool shouldRepaint(covariant _FilmStripPainter oldDelegate) => oldDelegate.offset != offset;
}